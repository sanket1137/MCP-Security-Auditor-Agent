...
async function createReportFile(reportContent: string, reportType: string): Promise<string> {
...
function generateAuditReportHTML(): string {
...
function generateComprehensiveReportHTML(servers: any[], reportData: any): string {
    const timestamp = new Date().toLocaleString();
    return `<!DOCTYPE html>
<html lang="en">
<head>
                    ...
                    <div class="server-card">
                        ...
                        <div style="margin-top: 15px;">
                            ...
                            <div class="compliance-badges">
                                ${server.security.compliance.map((comp: string) => `<span class="badge">${comp}</span>`).join('')}
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(
                    `Discovered ${simulatedServers.length} MCP servers`,
                    'View Details'
                ).then(selection => {
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(
                    ...
                    'View Report', 'Open File Location'
                ).then(selection => {
    ...
    const listServersWithAuditCommand = vscode.commands.registerCommand('mcpAuditor.listServersWithAudit', async () => {
        ...
        vscode.window.withProgress({
            ...
            try {
                ...
                vscode.window.showInformationMessage(
                    `Analysis complete! ${servers.length} servers analyzed`,
                    'View JSON Report', 'View HTML Report', 'Open File Location'
                ).then(selection => {
